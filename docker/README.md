...
```
docker build --build-arg VERSION=5.0.8 -t phoss-smp-release-binary-xml-5.0.8 -f Dockerfile-release-binary-xml .
```
...
```
docker run -d --name phoss-smp-release-binary-xml-5.0.8 -p 8888:8080 phelger/smp:5.0.8
docker stop phoss-smp-release-binary-xml-5.0.8
docker rm phoss-smp-release-binary-xml-5.0.8
```
