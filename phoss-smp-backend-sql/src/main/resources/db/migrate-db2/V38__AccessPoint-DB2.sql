...
-- because it is unique by definition. CLOB values cannot be compared directly in
-- DB2, so they are cast to VARCHAR for the comparison.
INSERT INTO smp_access_point (id, endpointReference, certificate)
  ...
  WHERE e.id = (SELECT MIN(e2.id)
                ...
                WHERE e2.endpointReference = e.endpointReference
                  AND CAST(e2.certificate AS VARCHAR(20000)) = CAST(e.certificate AS VARCHAR(20000)));
...
UPDATE smp_endpoint e
  SET accessPointID = (SELECT MIN(ap.id)
                       ...
                       WHERE ap.endpointReference = e.endpointReference
                         AND CAST(ap.certificate AS VARCHAR(20000)) = CAST(e.certificate AS VARCHAR(20000)));
